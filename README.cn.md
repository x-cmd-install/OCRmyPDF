# OCRmyPDF

[English version](./README.md)

OCRmyPDF adds an OCR text layer to scanned PDF files, allowing them to be searched

![OCRmyPDF](https://repo.x-cmd.io/OCRmyPDF.svg?lang=zh)

## 安装

```sh
x install OCRmyPDF
```

## 代码洞察

合计: **36,635** 行代码（覆盖前 5 种语言、共 **189** 个文件）。

| 语言 | 代码 | 注释 | 空行 | 文件数 |
|------|-----:|-----:|-----:|------:|
| Python | 34,891 | 2,683 | 6,930 | 179 |
| Svg | 583 | 0 | 0 | 5 |
| Toml | 373 | 31 | 50 | 3 |
| Bash | 325 | 20 | 49 | 1 |
| Fish | 165 | 2 | 27 | 1 |

## 源代码

- **上游仓库**: <https://github.com/ocrmypdf/OCRmyPDF>
- **官网**: <http://ocrmypdf.readthedocs.io/>
- **许可证**: MPL-2.0

## 发布

- **最新版本**: `v17.11.0` (2026-08-28)
- **最近提交**: 2026-09-08
- **Release 含资产**: 4 个

## 流行度

- **Star**: 34,714 · **Fork**: 2,405 · **开放 issue**: 1,364 · **贡献者**: 120

## 累计统计

- **发布数**: 63 · **已合并 PR**: 207 · **开放 PR**: 6 · **已关闭 issue**: 1271 · **开放 issue**: 93 · **提交数**: 4488

## 最近活动

| 时间窗口 | 起始 | 发布 | 已合并 PR | 开放 PR | 已关闭 issue | 开放 issue | 提交 |
|---|---|---:|---:|---:|---:|---:|---:|
| 30d | 2026-08-12 | 1 | 5 | 3 | 2 | 2 | 31 |
| last60d | 2026-07-13 | 11 | 10 | 3 | 10 | 3 | 70 |
| 90d | 2026-06-13 | 11 | 19 | 3 | 12 | 4 | 108 |
| last180d | 2026-03-15 | 12 | 31 | 3 | 35 | 7 | 167 |
| 360d | 2025-09-16 | 20 | 50 | 4 | 80 | 10 | 397 |
| last720d | 2024-09-21 | 30 | 79 | 6 | 169 | 23 | 620 |

## Release 资产

| 资产 | 大小 | 目标平台 |
|------|-----:|----------|
| [ocrmypdf-17.11.0-py3-none-any.whl](https://github.com/ocrmypdf/OCRmyPDF/releases/download/v17.11.0/ocrmypdf-17.11.0-py3-none-any.whl) | 522.4 KiB | `other` |
| [ocrmypdf-17.11.0-py3-none-any.whl.sigstore.json](https://github.com/ocrmypdf/OCRmyPDF/releases/download/v17.11.0/ocrmypdf-17.11.0-py3-none-any.whl.sigstore.json) | 10.9 KiB | `other` |
| [ocrmypdf-17.11.0.tar.gz](https://github.com/ocrmypdf/OCRmyPDF/releases/download/v17.11.0/ocrmypdf-17.11.0.tar.gz) | 7.2 MiB | `native/unknown` |
| [ocrmypdf-17.11.0.tar.gz.sigstore.json](https://github.com/ocrmypdf/OCRmyPDF/releases/download/v17.11.0/ocrmypdf-17.11.0.tar.gz.sigstore.json) | 11.0 KiB | `other` |

## 改进这些数据

OCRmyPDF 的安装元数据由 [x-cmd/install](https://github.com/x-cmd/install) 索引维护——这是一份由 x-cmd 在安装时读取的精选 YAML 包列表。如果 `OCRmyPDF` 缺失、过期，或安装行为有问题，欢迎在该 repo 提 issue 或 PR：

- **提交 issue**: <https://github.com/x-cmd/install/issues/new>
- **编辑包条目**: <https://github.com/x-cmd/install/edit/main/OCRmyPDF.yml>（或索引实际使用的路径）

本页面的数据（card / loc / scorecard / release）由 [x-cmd-install-action](https://github.com/x-cmd-install/x-cmd-install-action) 自动采集，每日重新生成。**安装行为**（版本选择、平台差异、依赖处理）的改进应提交到上游索引。

_数据快照: `data/card/260911.yml` · 2026-09-11T04:26:01Z._
